...
##orbF = cv2.ORB_create()          
orbF = cv2.ORB_create(scoreType=1)  # scoreType=1로 FAST_SCORE를 사용하여 orbF 객체 생성.
kp= orbF.detect(gray)
print('len(kp)=', len(kp))
...
#3
kp = sorted(kp, key=lambda f: f.response, reverse=True) # kp를 반응값 기준으로 내림차순 정렬.
filtered_kp = list(filter(lambda f: f.response>50, kp)) # 반응값이 50보다 작은 특징점들을 제거.
filtered_kp = filteringByDistance(kp, 10)   # distE=10 이내의 특징점들 제거.
print('len(filtered_kp)=', len(filtered_kp))

kp, des = orbF.compute(gray, filtered_kp)   # gray에서 특징점 filterd_kp를 이용하여 ORB 디스크립터 des를 계산.
...
for f in filtered_kp:
    ...
    box = cv2.boxPoints(rect).astype(np.int32)  # rect의 모서리 좌표를 box에 읽음.
