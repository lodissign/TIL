...
SAVER_DIR = "model"
saver = tf.train.Saver()  # tf.train.Saver() 클래스를 saver라는 이름으로 선언.
checkpoint_path = os.path.join(SAVER_DIR, "model")  # 모델과 파라미터를 저장할 경로인 checkpoint_path 선언(스크립트가  실행되는 디렉토리 아래에 있는 model이라는 폴더 아래에 model(SAVER_DIR)이라는 이름으로 모델과 파라미터를 저장하도록 checkpoint_path 지정)
ckpt = tf.train.get_checkpoint_state(SAVER_DIR) # 모델과 파라미터가 지정된 폴더 경로에 있는 checkpoint 파일의 정보를 읽어옴.
# checkpoint 파일에는 저장한 모든 체크포인트들이 all_model_checkpoint_paths라는 이름으로, 
# 가장 최근에 저장된 체크포인트가 model_checkpoint_path 라는 이름으로 기록되어 있음.
...
with tf.Session() as sess:
  ...
  for step in range(10000):
    ...
    if step % 100 == 0:      # 100회 반복마다 saver.save 함수를 호출해서 모델과 파라미터들을 저장. model 폴더에 "model(지정해준 파일 이름)-9500(반복 횟수)" 라는 이름으로 모델과 파라미터들이 파일 형태로 저장.
