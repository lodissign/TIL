...
src = np.array([[2, 2, 4, 4],
                ...
                [4, 4, 4, 4],
                [4, 4, 4, 4]
              ], dtype=np.uint8)  # 화소값의 범위가 [2,4]로 좁은 범위에 있어 어두운 영상이다.
#1
dst = cv2.equalizeHist(src) # 저대비 영상 src를 히스토그램 평활화한 결과 영상인 dst는 화소값의 범위가 [0, 255]로 넓은 고대비 영상이다.
print('dst =', dst)

#2
# 히스토그램 평활화 알고리즘 직접 구현.
'''
