...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  # 사전 학습된 BOTTLENECK을 인풋으로 받고, 새로 파인 튜닝하는 레이블을 타겟 데이터로 받기 위한 bottleneck_input, groud_truth_input 플레이스홀더 정의.
  ...
  # 새로 추가하는 Softmax 층의 Weight, Bias 변수 layer_weights, layer_biases를 정의.
  ...
  with tf.name_scope(layer_name):
    ...
    with tf.name_scope('Wx_plus_b'):
      logits = tf.matmul(bottleneck_input, layer_weights) + layer_biases  # Wx + b 연산 결과인 logits 연산 정의.
      tf.summary.histogram('pre_activations', logits)

  # 새로 추가한 Softmax 레이어의 출력값을 계산합니다.
  final_tensor = tf.nn.softmax(logits, name=final_tensor_name)  # softmax 함수를 적용한 최종 출력값.
  tf.summary.histogram('activations', final_tensor)

  # 크로스 엔트로피 손실함수를 정의합니다.
  with tf.name_scope('cross_entropy'):
    cross_entropy = tf.nn.softmax_cross_entropy_with_logits(  # 크로스 엔트로피 손실 함수 정의.
        labels=ground_truth_input, logits=logits)
    with tf.name_scope('total'):
      cross_entropy_mean = tf.reduce_mean(cross_entropy)
  tf.summary.scalar('cross_entropy', cross_entropy_mean)

  # 옵티마이저(GradientDescentOptimizer)를 정의하고, 파라미터를 업데이트하는 train_step 연산을 정의합니다.
