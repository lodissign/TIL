...
# 학습에 필요한 설정값들을 정의합니다.
# 사전 학습으로 MNIST 재구축을 위한 오토인코더를 학습시키고,
# 파인 튜닝을 이용해서 오토인코더의 파라미터를 업데이트.
...
def build_softmax_classifier(x):
  W_softmax = tf.Variable(tf.zeros([hidden2_size, 10]))    # 소프트맥스 분류기는 원본 MNIST 이미지(=784차원) 대신 오토인코더의 압축된 특징(=64 차원)을 입력값으로 받음.
